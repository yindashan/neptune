...
function initUI(_box){
	...
	//dialogs,自定义
	$("a[target=dialogedit]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var rel = $this.attr("rel") || "_blank";
			...
			url_str = "/asset/get_edit_status/" + url_array[url_array.length-2] + "/";
			...
			$.ajax({
		   		...
		   		success:function(data) {
		    		...
		    		//alert(data);
		    		if(data == "1") {
		    			alertMsg.info("当前记录已被锁定不能修改！"); //error、warn、info
		    		} else {
		    			DWZ.debug(url);
						if (!url.isFinishedTm()) {
							alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
	...
	//dialogadvanced,自定义
	$("a[target=dialogadvanced]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var rel = $this.attr("rel") || "_blank";
			...
			//var url = unescape(url).replaceTmById($(event.target).parents(".unitBox:first"));
			//var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
			DWZ.debug(url);
			if (!url.isFinishedTm()) {
				alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
	...
	//dialogs,自定义
	$("a[target=dialogsearch]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var rel = $this.attr("rel") || "_blank";
			...
			$.ajax({
		   		...
		   		success:function(data) {
		   			...
		   			//alert(data);
		    		if(data == "false") {
		    			alertMsg.info("API服务不可用！"); //error、warn、info
		    		} else {
		    			DWZ.debug(url);
						if (!url.isFinishedTm()) {
							alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
	...
	// 自定义
	if ($.fn.lookupapi) $("a[lookupGroupapi]", $p).lookupapi();
